 ...
 * `refetch` immediately. A top-level page (download/export, local `npm run preview`)
 * is not framed by any preview, so no token can ever arrive: the hook reports
 * `not_embedded` without probing. Any framed page probes, even when the parent
 * origin cannot be resolved (empty referrer, no `ancestorOrigins`): the token
 * comes through the preview proxy, and the bridge event is only the faster signal.
 */

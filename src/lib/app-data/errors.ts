...
const MESSAGE_RULES: readonly MessageRule[] = [
  { needles: ["not_connected", "failed_precondition"], kind: "not_connected", message: "Connect this connector in Grok to load your data." },
  { needles: ["scope_denied"], kind: "scope_denied", message: "This view isn't available — the app requested a tool outside its grant." },
  { needles: ["access_denied"], kind: "access_denied", message: "You don't have access to this data." },
];
...
export function classifyCallToolError(result: CallToolResult): CallToolErrorState | null {
  ...
  if (raw.includes("missing_connector_token")) {
    return { kind: "error", message: "Open this app from Grok to load your data.", detail };
  }
